...
class _CarPriceAppState extends State<CarPriceApp> {
  ...
  void showPredictionOverlay(BuildContext context, double predictedPrice, double accuracy, double? realPrice) {
    if (realPrice==null)return;

    double minValue = 0;
    double maxValue = realPrice; 
    double percentage = (predictedPrice / realPrice).clamp(0.0, 1.0);
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                "\$${predictedPrice.toStringAsFixed(0)}K",
                style: TextStyle(
                  fontSize: 26,
...
Future<void> _showPlot(String plotName) async {
  ...
  return;
    ...
    double? realPrice = double.tryParse(priceController.text);
    ...
    try {
      ...
      if (responseData.containsKey('predicted_Price')) {
        ...
        showPredictionOverlay(context, predictedPrice, accuracy, realPrice);
      } else {
